...
if(isset($_POST['filter'])){
  ...
  $query = "SELECT*FROM `checkin` WHERE labassign='ncr' AND CONCAT(`No`,`user`,`role`,`labassign`,
    `Date_In`,`Date_Out`,`timein`,`timeout`) LIKE '%".$search."%'";  
  $run_query = mysqli_query($conn, $query);
}
else{
  $query = "SELECT*FROM `checkin` WHERE labassign='ncr'";
  $run_query = mysqli_query($conn, $query);
}
    ?>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-primary">
  <a class="navbar-brand" href="admin.php">
    ...
    Computer Engineering Laboratory Officer NCR
...
<div class="search-container">
    <form action="searchncr.php" method="POST">
	...
	<thead>
               ...
               <?php
                echo"<tr alight=center>
                  ...
                  <td align=center><a class='btn btn-primary' href=\"ncr_print.php?user=$row[user]\">Print Attendance</a></td>
