       ...
       $user=$_SESSION['user'];
        ...
        class myPDF extends FPDF
        {
          ...
          function viewTable($conn,$user){
                ...
                $query=mysqli_query($conn,"SELECT checkin.No,checkin.user,checkin.role,checkin.labassign,checkin.Date_In,checkin.Date_Out,checkin.timein,checkin.timeout,checkin.time_dur,schedule.Monday,schedule.Tuesday,schedule.Wednesday,schedule.Thursday,schedule.Friday FROM checkin inner join schedule WHERE checkin.user='$user' AND checkin.role=schedule.role AND checkin.labassign=schedule.labassign ORDER BY Date_In");
                while($data = mysqli_fetch_array($query)){
                  ...
                  $this->Cell(25,10,$data['labassign'],1,0,'P');                   
